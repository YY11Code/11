...
### 输出目录 `<predict-dir>/analysis_failures/`

**PyMOL 配色**（与 `.pse` / `all.pdf` 里结构颜色一致）：

| 色块 | PyMOL | 含义 |
|:--:|:-----:|------|
| <span style="color:#2563eb">**■ GT 抗体 / GT CDR**</span> | `blue` | Ground truth 抗体链，或 GT 的 CDR 环 |
| <span style="color:#0891b2">**■ GT 抗原**</span> | `cyan` | Ground truth 抗原链 |
| <span style="color:#dc2626">**■ pred 抗体 / pred CDR**</span> | `red` | 预测抗体链，或 pred 的 CDR 环 |
| <span style="color:#ea580c">**■ pred 抗原**</span> | `orange` | 预测抗原链 |
| <span style="color:#9ca3af">**■ 背景**</span> | `grey70` | 非焦点区域（半透明 cartoon） |

---

**`failed_interfaces.csv`**  
失败条目主表（**N** 行，N = DockQ < 0.23 的 interface 数）。  
列含：`interface_name`, `dockq_score`, `antigen_rmsd`, `cdr1/2/3_rmsd`, 结构路径与链号映射等。

**`category_summary.pdf`**  
**5** 根柱状图：DockQ 失败总数 + 抗原 / CDR1 / CDR2 / CDR3 中 RMSD > 2 Å 各多少条。
...
| | |
|--|--|
| **对齐** | pred 整体按 **OST 变换矩阵**叠到 GT（`detail/*_structure_ost.json`） |
| **高亮** | <span style="color:#2563eb">GT抗体</span> · <span style="color:#0891b2">GT抗原</span> · <span style="color:#dc2626">pred抗体</span> · <span style="color:#ea580c">pred抗原</span> · <span style="color:#9ca3af">其余背景</span> |
| **`.pse`** | **N** 个，如 `8ivx-assembly1_A_B.pse`（PyMOL 交互查看） |
| **`all.pdf`** | **N** 页；页标 **interface 名 + DockQ** |
...
| **高亮** | <span style="color:#0891b2">GT抗原</span> · <span style="color:#ea580c">pred抗原</span> · <span style="color:#9ca3af">其余背景</span>（含抗体） |
| **`.pse`** | **N** 个（每条失败条目都有，**不论 RMSD 大小**） |
| **`all.pdf`** | **M₁** 页；仅 `antigen_rmsd > 2 Å`；页标 **interface 名 + DockQ + Ag RMSD** |
...
| | |
|--|--|
| **对齐** | pred 抗体链 CA → GT **CDR1** 残基 CA（IMGT 标注） |
| **高亮** | <span style="color:#2563eb">GT CDR1</span> · <span style="color:#dc2626">pred CDR1</span> · <span style="color:#9ca3af">其余背景</span> |
| **`.pse`** | **N** 个 |
| **`all.pdf`** | **M₂** 页；仅 `cdr1_rmsd > 2 Å`；页标 **interface 名 + DockQ + CDR1 RMSD** |
...
| | |
|--|--|
| **对齐** | pred 抗体链 CA → GT **CDR2** 残基 CA（IMGT 标注） |
| **高亮** | <span style="color:#2563eb">GT CDR2</span> · <span style="color:#dc2626">pred CDR2</span> · <span style="color:#9ca3af">其余背景</span> |
| **`.pse`** | **N** 个 |
| **`all.pdf`** | **M₃** 页；仅 `cdr2_rmsd > 2 Å`；页标 **interface 名 + DockQ + CDR2 RMSD** |
...
| | |
|--|--|
| **对齐** | pred 抗体链 CA → GT **CDR3** 残基 CA（IMGT 标注） |
| **高亮** | <span style="color:#2563eb">GT CDR3</span> · <span style="color:#dc2626">pred CDR3</span> · <span style="color:#9ca3af">其余背景</span> |
| **`.pse`** | **N** 个 |
| **`all.pdf`** | **M₄** 页；仅 `cdr3_rmsd > 2 Å`；页标 **interface 名 + DockQ + CDR3 RMSD** |

> **N** = 失败条目总数 · **M₁–M₄** = 各目录 `all.pdf` 页数（≤ N）  
